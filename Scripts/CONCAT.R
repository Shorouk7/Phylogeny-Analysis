...
# - Multiple sequence alignment (COI+NDI gene)
...
writeXStringSet(aligned_sequences, "concat.aligned_sequences.fasta")
...
aligned <- readDNAStringSet("concat.aligned_sequences.fasta")
...
# Ensure dist_matrix is a valid distance matrix ( just to see the next code saving)
...
pheatmap(as.matrix(dist_matrix),
         ...
         angle_col = "45",
         border_color = NA,
         filename = "concat_heatmap_output.png",  # Save to file to avoid display cropping
...
tree <- read.tree("DATA/tree/conct.nwk")
...
node1_tips <- sanitize_tips(1,8, tree_rooted, tip_data)     # A.amoreuxi (M/A)
node2_tips <- sanitize_tips(9, 10, tree_rooted, tip_data)     # A.amoreuxi (E)
node3_tips <- sanitize_tips(11, 12, tree_rooted, tip_data)   # A.bicolor
node4_tips <- sanitize_tips(13, 14, tree_rooted, tip_data)   # A.bicolor Egy + A.crassicauda Egy
node5_tips <- sanitize_tips(15, 15, tree_rooted, tip_data)   # A.crassicauda India
node6_tips <- sanitize_tips(16, 16, tree_rooted, tip_data)   # Leiurus + Buthus
...
if (!is.na(node5)) p <- p + geom_hilight(node = node5, fill = "red", alpha = 0.3, extend = 0.3)
...
dev.new()
print(p)
ggsave("concat.tree_plot.png", plot = p, width = 10, height = 8)
